...
No ambiente de desenvolvimento local, a configuração do Supabase foi ajustada para desativar o registro público de usuários, mantendo o provedor de e-mail ativo exclusivamente para login e convites. Templates de e-mail locais foram criados para injetar corretamente o `token_hash` nas URLs de callback. Para suportar a execução contínua da suíte de testes E2E, o limite de envio de e-mails locais foi aumentado. O runner de testes (`scripts/run-auth-e2e.mjs`) foi aprimorado para injetar variáveis de ambiente no processo de build do Next.js através de um arquivo `.env.local` temporário. A suíte E2E em si foi atualizada para interagir com a API Mailpit do Supabase CLI, validando de forma abrangente todos os fluxos de autenticação, desde redirecionamentos e persistência de sessão até o controle de acesso e interceptação de e-mails locais.
...
| **Testes E2E** | Aprovado | 10 cenários concluídos, garantindo cobertura completa dos fluxos de autenticação integrados ao Mailpit local. |
