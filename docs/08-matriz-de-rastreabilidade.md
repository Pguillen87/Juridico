...
| O-004 | RF-011 | US-032 | `weekly_report` | Reports/backend | 12 | T-032 weekly | Geração idempotente por cliente/período, período `[start,end)`, sexta 17:00 `America/Sao_Paulo`, cutoff UTC e conteúdo congelado; pgTAP, unit, E2E e concorrência | IMPLEMENTADO/TESTADO |
| O-004 | RF-011 | US-033 | `report_process` | Reports/UI | 12 | T-033 by process | Projeção persistida por relatório, processo comprovado antes do cutoff, isolamento por `office_id`, detalhe/lista e pgTAP/E2E | IMPLEMENTADO/TESTADO |
| O-004 | RF-011 | US-034 | `report_party` | Reports/UI | 12 | T-034 by party | Projeção N:N congelada, ambiguidade sem confirmação de identidade, pendência manual, pgTAP e E2E | IMPLEMENTADO/TESTADO |
| O-004 | RF-011 | US-035 | `report_version` | Reports/UI | 12 | T-035 edit | Edição editorial allowlisted, submissão, devolução e restauração como nova versão; RPCs, pgTAP e E2E | IMPLEMENTADO/TESTADO |
| O-004 | RF-011 | US-036 | `report_version` | Reports/backend | 12 | T-036 version | Histórico append-only, versões técnicas/editoriais/restored, hash de conteúdo/manifesto e concorrência sem sobrescrita | IMPLEMENTADO/TESTADO |
| O-004 | RF-011 | US-037 | `weekly_report` | Authorization | 12 | T-037 approval | D-022 revalidada no banco; reviewer sem aprovação; lawyer aprova/cancela; hash recalculado, auditoria atômica e cancelamento terminal | IMPLEMENTADO/TESTADO |
| O-004 | RF-012 | US-038 | `report_artifact` | PDF/Storage | 13 | T-038 PDF hash | `17_phase_13_pdf_delivery.test.sql`, `pdf-renderer.test.ts`: PDF `%PDF-`, SHA-256, snapshot, tamanho, bucket/URI privados e artefato somente após aprovação | IMPLEMENTADO — evidência corretiva local pendente |
| O-004 | RF-012 | US-039 | `email_delivery` | Delivery | 13 | T-039 send | `17_phase_13_pdf_delivery.test.sql`, `email-provider.test.ts`: contato confirmado, autorização lawyer, idempotência, estados e resultado `unknown_outcome`; provider real não chamado | IMPLEMENTADO — evidência corretiva local pendente; transporte real deferido |
| O-004 | RF-012 | US-040 | `email_delivery`, `report_artifact` | Delivery | 13 | T-040 immutable copy | Hash aprovado, hash do artefato, URI privada, tentativas append-only, retry/reconcile/resend e auditoria allowlisted | IMPLEMENTADO — evidência corretiva local pendente; storage remoto e e-mail real deferidos |
| O-005 | RF-013 | US-041 | `audit_log` | Audit | 3–14 | T-041 audit | RPCs F13 gravam eventos allowlisted na mesma transação; rollback de auditoria e imutabilidade cobertos por `17_phase_13_pdf_delivery.test.sql` | IMPLEMENTADO — evidência corretiva local pendente; retenção/expurgo deferidos |
...
As histórias `Must` possuem ao menos um teste associado. Na Fase 5, US-004 e US-005 foram implementadas e testadas por RPCs transacionais, RLS, pgTAP, unitários e E2E. Na Fase 6, US-006 a US-010 foram implementadas e testadas com RPCs, RLS, pgTAP, unitários, E2E e concorrência. Na Fase 9, o teste PostgreSQL real demonstra scheduler idempotente, claim exclusivo, conclusão sem duplicação, lease antiga rejeitada e nova lease funcional; `scripts/check-phase9-migration-history.sh` demonstra que 00011 é byte a byte igual a `2904185` e que o hardening existe somente em 00012. Na Fase 10, US-019 a US-022 foram implementadas com comparador determinístico, persistência append-only, fonte única do diff, `detected_change` mínimo, RLS/grants, auditoria atômica, pgTAP e concorrência real. Na Fase 11, US-023 a US-031 foram cobertas no escopo aprovado com incidentes, timeline append-only, filtros server-side incluindo tentativa, RPC de reprocessamento com D-022, outbox sintética, deduplicação lógica, RLS/grants, pgTAP, unitários, E2E e concorrência. Na Fase 12, US-032 a US-037 foram implementadas e testadas com migration aditiva, conteúdo congelado por cliente/período, projeções de processo/parte, edição allowlisted, histórico de versões, aprovação com hash recalculado, cancelamento terminal, RLS/grants, auditoria atômica, pgTAP, unitários, E2E e oito cenários de concorrência. Na Fase 13, US-038 a US-041 foram implementadas e testadas no escopo local/sandbox com artefato PDF validado por hash, bucket privado, contatos confirmados, autorização idempotente, estados de delivery, tentativas/reconciliação, cópia imutável de hashes/URI e auditoria atômica. Provider real, e-mail real, storage remoto, produção, staging e retenção/expurgo permanecem explicitamente deferidos. O corretivo da Fase 11 registrou que a primeira publicação da 00003 continha dois `RAISE` sintaticamente inválidos e não era aplicável a banco novo; uma exceção única autorizada corrigiu somente esses dois comandos, e o novo blob canônico passou a ser imutável. O hardening posterior permanece na 00004. US-011 permanece parcial/deferida: existe somente estado estrutural `paused`, sem provider, scheduler, fila, capability ou job.
...
O corretivo não amplia D-022, não cria entrada manual operacional e não antecipa a Fase 12. A primeira versão publicada da migration 00003 foi defeituosa e não constituía baseline válida de banco; a exceção única autorizada alterou somente os dois `RAISE` com `TG_TABLE_NAME`. O novo blob corrigido da 00003 é a versão canônica imutável, e o hardening funcional posterior permanece exclusivamente na 00004. A Fase 11 não implementa envio real: US-030 permanece parcial/deferida quanto a entrega externa e US-031 comprova somente deduplicação lógica sintética.
...
A Fase 10 adiciona `src/lib/comparison/` com canonicalização determinística, versionamento interno allowlisted `comparison-v1`, diff técnico e hashes estáveis. `process_comparison` é a fonte única e imutável do resultado completo; `detected_change` é mínimo, só referencia comparações `changed` e não duplica `normalized_diff` nem `changed_fields`. A migration `20260827000001_phase_10_comparison_detection.sql` foi restaurada byte a byte ao commit publicado `7527d6a9`; o hardening posterior está exclusivamente em `20260827000002_phase_10_comparison_hardening.sql`, com RPC v2 de retorno completo, leitor compatível, `SECURITY DEFINER`, `search_path` fixo, RLS, grants backend-only, lock curto por processo, auditoria allowlisted na mesma transação e rollback integral se a auditoria falhar.

Os testes `supabase/tests/database/13_phase_10_comparison_detection.test.sql` e `14_phase_10_compatible_baseline.test.sql` cobrem 43 assertions após reset limpo: 33 do contrato comparativo e 10 do cenário A-B-C. O segundo prova o cenário `A` compatível → `B` posterior incompatível → `C` atual compatível: C escolhe A; também cobre ausência de baseline compatível, provider/source incompatível, normalizer incompatível, snapshot inválido e outro office. O teste real `supabase/tests/concurrency/test_phase10_comparison.sh` passou com duas comparações concorrentes, um replay, duas linhas em `process_comparison`, uma linha em `detected_change`, zero colunas de diff duplicadas e rejeição de versão não allowlisted. O gate histórico do App CI prova a integridade da 00001 e a exclusividade do hardening na 00002. Falhas do provider continuam sem snapshot e nunca são convertidas em `unchanged`; nenhuma UI, notificação, relatório, PDF ou Fase 11 foi iniciada.
