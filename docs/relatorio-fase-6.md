...
As migrations incrementais adicionadas foram `20260826000006_phase_6_processes_csv.sql` e `20260826000007_phase_6_preview_read.sql`. A primeira cria `legal_process`, `process_party`, `process_import_preview`, constraints, índices, RLS, grants mínimos, helper CNJ, RPCs de domínio, staging privado, TTL de 30 minutos e auditoria atômica. A segunda expõe exclusivamente a leitura controlada do preview criado pelo próprio actor/office por meio de RPC; não concede DML direto ao staging.
...
O reset do Supabase aplicou todas as migrations da baseline, as migrations da Fase 5 e as duas migrations da Fase 6. O `db:lint` terminou com **No schema errors found**. A geração dos tipos Supabase foi executada e formatada. O `db:test` aprovou a suíte existente de 4C/5 e o novo arquivo `09_phase_6_processes_csv.test.sql`, com as 43 asserções declaradas da Fase 6, totalizando **202 asserções pgTAP** no pacote.
...
| pgTAP | Aprovado; 202 asserções no pacote |
...
O workflow do App CI deve incluir `phase-6-processes-csv` no trigger de push, preservar os gates da baseline e executar `scripts/test-phase6-concurrency.ps1` após as concorrências existentes da Fase 5. O workflow continua read-only: não contém `git add`, `git commit` ou `git push`.

Antes do push final deverão ser verificados `git diff --check`, working tree, ausência de artefatos proibidos e a presença somente dos arquivos intencionais da Fase 6. Após um único commit incremental, o App CI deverá ser aguardado até `completed / success`. A conclusão formal da Fase 6 somente ocorrerá depois desse resultado verde e da inspeção factual do SHA executado.

## 8. Documentação e critérios de parada

A matriz `docs/08-matriz-de-rastreabilidade.md` foi atualizada para marcar US-006 a US-010 como `Implemented/Tested` e US-011 como `Partial/Deferred`, sem declarar provider ou monitoramento real. Este relatório e o plano aprovado deverão ser publicados juntos no pacote final.
...
**Estado do relatório antes do push:** implementação local concluída; gates locais possíveis aprovados; validação do App CI ainda pendente de publicação do pacote completo.
